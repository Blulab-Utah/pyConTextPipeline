...
@author: Originally written by Alec Chapman; edited by Danielle Mowery; edited by Garrett Cole (Attributes)
"""
...
class mentionAnnotation(object):
    def __init__(self, tagObject, textSource=None, mentionClass=None, mentionAttribute=None, mentionid=None, annotatorid='pyConText_FINDER', span=None, start=None, end=None, spannedText=None, creationDate=None, XML=None, MentionXML=None):
        """Creates an annotation of Object"""
        self.textSource = textSource
        self.mentionid = str(mentionid)
        self.mentionClass = mentionClass
        self.mentionAttribute = mentionAttribute
    ...
    def getMentionAttribute(self):
        return self.mentionAttribute
...
def createAnnotation(mode, ct, file_name, term, span, mention_class, mention_attribute=None): #eventually mention_class will be defined by the logic
    """Takes a ConTextMarkup object and returns a list of annotation object"""
    if mode == 'combined':
        annotation = mentionAnnotation(tagObject=ct, textSource=file_name, mentionClass=mention_class, mentionAttribute=None, mentionid=ct, spannedText=term, span=eval(span), start=eval(span)[0], end=eval(span)[1]) #MADE THIS THE DOCSPAN
    elif mode == 'separate':
        annotation = mentionAnnotation(tagObject=ct, textSource=file_name, mentionClass=mention_class, mentionAttribute=mention_attribute, mentionid=ct, spannedText=term, span=eval(span), start=eval(span)[0], end=eval(span)[1]) #if separate, mention_class is a tuple of (Target, Modifier)
...
def writeKnowtator(mode, annotations, save_dir, text_source): #text_source should be read automatically from the XML string
    ...
    text_source = text_source+'.txt' #documents read from db don't have an extension, so add .txt here
...
#     return os.path.join(outpath,text_source+'.knowtator.xml')
    ...
    fout=open("%s/%s.knowtator.xml"%(save_dir,text_source),"w")
    ...
    for annotation in annotations:
        ...
        if mode == 'separate':
            #attribute
            outStringSlotMention+="""    <stringSlotMention id="EHOST_Instance_%s">\n"""%str(attr_ct)
            outStringSlotMention+="""        <mentionSlot id="modifier" />\n"""
            outStringSlotMention+="""        <stringSlotMentionValue value="%s" />\n"""%annotation.getMentionAttribute()
            outStringSlotMention+="""    </stringSlotMention>\n"""

        #create Knowtator mention class
        outStringClsMent+="""    <classMention id="pyCONTEXT_Instance_%s">\n"""%str(ct)
        if mode == 'separate':#associate attribute with class
            outStringClsMent+="""    <hasSlotMention id="EHOST_Instance_%s" />\n"""%str(attr_ct)
        ...
        ct+=1
        if mode == 'separate':#alternate instance numbers
            ct+=1
            attr_ct+=2
        a=outString+outStringSlotMention+outStringClsMent
        annotXML+=a
