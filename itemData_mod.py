...
class contextItem(object):
    ...
    def __unicode__(self):
        txt = """literal<<{0}>>; category<<{1}>>; re<<{2}>>; rule<<{3}>>; code<<{4}>>""".format(
            self.__literal, self.__category, self.__re, self.__rule, self.__code)
        return txt
...
def get_item_data(file_str):
    ...
    if file_name.endswith(".csv") or file_name.endswith(".tsv") or file_name.endswith(".yml"):
        ...
        if not os.path.exists(file_str):
            print('Warning: ' + file_str + ' not found.')
            return itemData()
        ...
        print("This input format is not supported. It can be either a path of csv, tsv or yaml file, or a string of corresponding file content.")
...
def instantiateFromYMLStr(content):
    context_items = [contextItem((d["Lex"],
                                  ...
                                  r"%s" % d["Regex"],
                                  d["Direction"],
                                  d["Code"])) for d in safe_load_all(content)]
    return context_items


def instantiateFromYMLtoitemData(_file):
    def get_fileobj(_file): 
        if len(urllib.parse.urlparse(_file).scheme) <= 1:
            _file = "file:///" + _file
